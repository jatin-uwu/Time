    ...
    return Controller.extend("timesheet.app.controller.FounderProjects", {
        ...
        onCreateProject: function () {
            ...
            Promise.all([
                ...
                var body =
                    "<div class='fpForm'>" +
                    "<label>Project Name *</label><input class='fpInput' id='pName'/>" +
                    ...
                    "<label>Description</label><textarea class='fmodTextarea' id='pDesc'></textarea>" +
                    ...
                    // ── Financial model ─────────────────────────────────────────────
                    "<div id='pFinWrap'><div class='fpRow'>" +
                    ...
                    "<label>Focus Areas (comma-separated)</label><input class='fpInput' id='pFocus' placeholder='SAP CAP Development, Fiori UI, Security Compliance'/>" +
                ...
                var m = FP.modal({ title: "Create Project", body: body, wide: true });
        ...
        onManageClients: function () {
            ...
            ppost("getClientMasters", {}).then(function (d) {
                ...
                var rows = clients.length ? clients.map(function (c) {
                    return "<tr><td><b style='color:#e6edf8'>" + esc(c.clientName) + "</b><div class='fdCardSub'>" + esc(c.companyName || "") + "</div></td>" +
                        ...
                        "<td><span style='color:" + (String(c.status).toLowerCase() === "active" ? "#34d399" : "#fb7185") + ";font-weight:700;font-size:0.8rem'>" + esc(c.status) + "</span></td></tr>";
                }).join("") : "";
                var body = "<div class='fpForm'>" +
                    "<button class='faBtn approve' id='cNew' style='margin-bottom:12px'>＋ New Client</button>" +
                    (rows ? "<table class='fpTable'><thead><tr><th>Client</th><th>Contact</th><th>Projects</th><th>Status</th></tr></thead><tbody>" + rows + "</tbody></table>" : "<div class='fdCardSub'>No clients yet. Create your first client.</div>") +
        ...
        onNewClient: function () {
            var that = this;
            var body = "<div class='fpForm'>" +
                "<label>Client Name *</label><input class='fpInput' id='clName'/>" +
                "<label>Company Name</label><input class='fpInput' id='clCo'/>" +
                "<div class='fpRow'><div><label>Contact Person</label><input class='fpInput' id='clContact'/></div>" +
                "<div><label>Phone</label><input class='fpInput' id='clPhone'/></div></div>" +
                "<label>Email * (client login identity)</label><input class='fpInput' id='clEmail' placeholder='contact@company.com'/>" +
                "<label>Notes</label><textarea class='fmodTextarea' id='clNotes'></textarea>" +
                "<div class='fmodFoot'><button class='faBtn ghost' id='cCancel'>Cancel</button><button class='faBtn approve' id='cSave'>Create Client</button></div></div>";
            var m = FP.modal({ title: "New Client", body: body });
            m.body.querySelector("#cCancel").addEventListener("click", m.close);
            m.body.querySelector("#cSave").addEventListener("click", function () {
                var g = function (id) { var el = m.body.querySelector(id); return el ? el.value.trim() : ""; };
                if (!g("#clName")) { FP.toast("Client Name is required.", false); return; }
                if (!g("#clEmail")) { FP.toast("Email is required.", false); return; }
                this.disabled = true; this.textContent = "Creating…";
                ppost("createClientMaster", { clientName: g("#clName"), companyName: g("#clCo"), contactPerson: g("#clContact"), phoneNumber: g("#clPhone"), email: g("#clEmail"), notes: g("#clNotes") }).then(function (res) {
                    m.close();
                    if (res && res.error) { FP.toast(res.error, false); return; }
                    FP.toast("Client created."); that.onManageClients();
                }).catch(function () { m.close(); FP.toast("Could not create client.", false); });
            });
        },
